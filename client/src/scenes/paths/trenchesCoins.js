...
// Payout multiplier derived from the coin's base speed. Previously EVERY
// coin paid the same per landing, which made the whole coin-choice
// mechanic hollow — picking a fast/risky coin was pure downside with no
// upside, so the rational choice was always the safest one. Scaling the
// reward with the risk is what actually makes the decision interesting.
// $STABLE (3.2) ~= 1.0x, $APEMAX (15.0) ~= 2.3x, $LEGEND (17.0) ~= 2.5x.
export function coinPayoutMultiplier(coin) {
  return 1 + (coin.base - 3.2) * 0.11;
}
